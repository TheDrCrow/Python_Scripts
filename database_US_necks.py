...
import csv
import re
conn = sqlite3.connect('nimreport.db')
...
c.execute(
    "SELECT uid, sex, age, referrer, procedure_code, IMPRESSION, PRIMARY_CONCLUSION , INFORMATION_EFF_SCORE FROM nimreports WHERE procedure_code REGEXP ?" , [r'\bUN.{2,3}'])

result=c.fetchall()

# Write the ourput to a file.
with open('All_Neck_US.txt', mode="wt", encoding='utf-8') as myfile:
    for lists in result:
...
# Write the ourput to a file.
# with open('', mode="wt", encoding='utf-8') as myfile:
#    for lists in result:
#        myfile.write('; '.join(str(list) for list in lists))
#        myfile.write('\n')