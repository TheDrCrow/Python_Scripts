...
c.execute(
    "SELECT uid, age, sex, referrer, in_patient, on_call, q_score, procedure_code, CLINICAL_TYPE, CLINICAL_DATA_ELEMENT, PROCEDURE, QUALITY, Limited_by, COMPARISON, PRIMARY_CONCLUSION, SECONDARY_CONCLUSION, INFORMATION_EFF_SCORE FROM nimreports ;" ) 

result=c.fetchall()

# Write the ourput to a file.
with open('nimreports_to5326.csv', mode="wt", encoding='utf-8') as myfile:
    reps = 0
    for lists in result:
...
# Write the ourput to a file.
# with open('xxxx', mode="wt", encoding='utf-8') as myfile:
#    for lists in result:
#        myfile.write('; '.join(str(list) for list in lists))
#        myfile.write('\n')